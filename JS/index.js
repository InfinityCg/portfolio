...
function fetchProjectData(projectName) {
    ...
    .then(([descriptionText, mediaText]) => {
        ...
        const isPortfolioProject = tags.includes("Portfolio"); // Check if "Portfolio" is in tags
        ...
        return { src: thumbnailUrl, alt: title, galleryPageUrl, hasMultipleImages, hasVideo, hasYouTube, hasSketchfab, bannerImageUrl, isPortfolioProject };
    })
...
fetchProjects().then(projects => {
    ...
    const fetchProjectDataPromises = projects.map(projectName => {
        return fetchProjectData(projectName).then(artwork => {
            // Only add projects to index.html if they are marked as "Portfolio"
            if (!isIndexPage || artwork.isPortfolioProject) {
